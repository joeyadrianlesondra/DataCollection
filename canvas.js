window.addEventListener('load', () => {
    ...
    /**
     * NEW HELPER FUNCTION: Calculates drawing-level aggregated metrics from stroke data.
     */
    function calculateDrawingMetrics(drawingData) {
        ...
        strokes.forEach(s => {
            // Only strokes with calculated duration/speed should be counted for the average
            if (s.duration > 0 || s.avgSpeed > 0) {
                totalDuration += s.duration;
                totalAvgSpeed += s.avgSpeed;
                numStrokes++;
            }
        });
        
        // Use the average pressure already calculated in getStrokeData (stored in drawingData.avgPressure)
        
        return {
            totalStrokes: strokes.length,
            // Calculate average duration across all valid strokes
            avgDuration: numStrokes > 0 ? totalDuration / numStrokes : 0,
            // Calculate average of the average speeds of each valid stroke
            avgSpeed: numStrokes > 0 ? totalAvgSpeed / numStrokes : 0, 
            // Use the overall drawing average pressure already calculated
            avgPressure: drawingData.avgPressure, 
            totalPoints: drawingData.totalPoints
        };
    }


    // Format stroke data for readable display (MODIFIED)
    function formatStrokeData(data) {
        ...
        output += "--- DRAWING METRICS SUMMARY ---\n";
        output += "------------------------------------------------------\n";
        
        data.forEach((drawing, dIndex) => {
            // Calculate the drawing-level metrics using the new function
            const metrics = calculateDrawingMetrics(drawing.strokeData);
            
            output += `Drawing ${dIndex + 1}: ${taskPrompts[dIndex]}\n`;
            output += `Total Strokes: ${metrics.totalStrokes}\n`;
            output += `Avg Stroke Duration: ${metrics.avgDuration.toFixed(0)} ms\n`;
            output += `Avg Stroke Speed: ${metrics.avgSpeed.toFixed(3)} px/ms\n`;
            output += `Avg Drawing Pressure: ${metrics.avgPressure.toFixed(3)}\n`;
            output += "------------------------------------------------------\n";
        });
        return output;
    }
    // --- Download functions retained as is ---
    ...
    // Return a copy of stroke data and calculate metrics (Retained as is)
    ...
    // --- 3. DRAWING & METRIC LOGIC (Retained as is) ---
    ...
    // --- 4. UI FLOW MANAGEMENT (Retained as is) ---
    ...
    // --- 5. INITIAL START SCREEN UI (Retained as is) ---
    ...
    // --- 6. CANVAS & TASK UI SETUP (Retained as is) ---
    ...
    // --- 7. FINAL SUBMISSION POPUP & DOWNLOAD (Retained as is) ---
    ...
    function showSubmissionPopup() {
        ...
        // View Data Button (Retained as is)
    ...
    // showDataViewer content is modified to use the simplified formatStrokeData
    function showDataViewer() {
        ...
        // 1. Text Data (Stroke Metrics) - MODIFIED TO USE SIMPLIFIED OUTPUT
        ...
        // 2. Image Thumbnails (Retained as is)
        ...
        // --- DOWNLOAD & CLOSE BUTTONS (Retained as is) ---
